...
def make_predictions(testing_nonzero_index, testing_nonzero_values, reverse_index, destination_matrix):
...
for user_idx in range(load_matrix.n_users):
	...
	if len(np.nonzero(load_matrix.test_data_matrix[user_idx,])[0]) > 0:
		...
		make_predictions(testing_nonzero_index, testing_nonzero_values, reverse_index, test_predictions)

	make_predictions(nonzero_index, nonzero_values, reverse_index, train_predictions)
